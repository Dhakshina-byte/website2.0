...
const AntigravityInner: React.FC<AntigravityProps> = ({
  ...
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      const t = seededRandom(i + 1) * 100;
      const factor = 20 + seededRandom(i + 2) * 100;
      const speed = 0.01 + seededRandom(i + 3) / 200;
      const xFactor = -50 + seededRandom(i + 4) * 100;
      const yFactor = -50 + seededRandom(i + 5) * 100;
      const zFactor = -50 + seededRandom(i + 6) * 100;

      const x = (seededRandom(i + 7) - 0.5) * width;
      const y = (seededRandom(i + 8) - 0.5) * height;
      const z = (seededRandom(i + 9) - 0.5) * 20;

      const randomRadiusOffset = (seededRandom(i + 10) - 0.5) * 2;
...
const Antigravity: React.FC<AntigravityProps> = props => {
  return (
    <Canvas camera={{ position: [0, 0, 50], fov: 35 }} style={{ touchAction: 'auto' }}>
